...
class ALSAlgorithm(val ap: ALSAlgorithmParams)
  ...
  def predict(model: ALSModel, query: Query): PredictedResult = {
    ...
    val queryList: Set[Int] = query.items.flatMap(model.itemStringIntMap.get).toSet

    val queryFeatures: Vector[Array[Double]] = queryList.toVector.flatMap { item => productFeatures.get(item) }

    val whiteList: Option[Set[Int]] = query.whiteList.map( set =>
      set.flatMap(model.itemStringIntMap.get)
    )
  ...
  def isCandidateItem(
    ...
    whiteList.forall(_.contains(i)) &&
    blackList.forall(!_.contains(i)) &&
    // discard items in query as well
    (!queryList.contains(i)) &&
    // filter categories
    categories.forall { cat =>
      items(i).categories.exists { itemCat =>
